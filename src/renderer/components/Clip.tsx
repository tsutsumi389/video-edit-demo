...
export function Clip({ clip, pixelsPerSecond, isSelected }: ClipProps) {
  ...
  const dragRef = useRef<{ startX: number; startPos: number; mode: 'move' | 'trim-left' | 'trim-right' } | null>(null);
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent, mode: 'move' | 'trim-left' | 'trim-right') => {
    e.preventDefault();
    e.stopPropagation();
    dispatch({ type: 'SELECT_CLIP', payload: { clipId: clip.id } });
